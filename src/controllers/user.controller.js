      ...
      const { count, rows: dataBiodata } = await models.Biodata.findAndCountAll({
			...
			const getResult = await Promise.all(dataBiodata.map(async val => {
				return {
					...
					kodePos: val.kodePos,
					pekerjaan: val.pekerjaan ? await _pekerjaanOption({ models, kode: val.pekerjaan }) : null,
					pekerjaanLainnya: val.pekerjaan === '18' ? val.pekerjaanLainnya : null,
					telp: val.telp,
					namaIstri: val.namaIstri,
					anak: await _anakOption({ models, idBiodata: val.idBiodata }),
					pekerjaanIstri: val.pekerjaanIstri ? await _pekerjaanOption({ models, kode: val.pekerjaanIstri }) : null,
					pekerjaanIstriLainnya: val.pekerjaanIstri === '18' ? val.pekerjaanIstriLainnya : null,
					...
					statusMeninggal: val.statusMeninggal,
